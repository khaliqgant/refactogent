  ...
  private displaySummary(report: ProjectHealthReport, verbose: boolean): void {
    ...
    if (report.recommendations.length > 0) {
      ...
      if (!verbose && report.recommendations.length > 5) {
        console.log(
          `  ... and ${report.recommendations.length - 5} more (use --verbose for full list)`
        );
      }
    }
    ...
    <div class="section">
        ...
        <table>
            ...
            ${Array.from(report.structure.filesByExtension.entries())
              .sort((a, b) => b[1] - a[1])
              .map(
                ([ext, count]) => `<tr><td>${ext || '(no extension)'}</td><td>${count}</td></tr>`
              )
              .join('')}
        </table>
    </div>
...
|------|---------|------|--------|
${report.dependencies
  .map(dep => `| ${dep.name} | ${dep.version} | ${dep.type} | ${dep.source} |`)
  .join('\n')}
`
    : ''
}

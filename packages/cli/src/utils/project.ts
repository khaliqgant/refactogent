...
export class ProjectAnalyzer {
  ...
  async analyzeProjectStructure(projectPath: string): Promise<ProjectStructure> {
    ...
    // Find largest files
    structure.largestFiles = allFiles.sort((a, b) => b.size - a.size).slice(0, 10);
  ...
  private async traverseDirectory(
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (
          ['node_modules', '.git', 'dist', 'build', '__pycache__', '.venv', 'venv'].includes(entry)
        ) {
          continue;
        }
  ...
  private calculateMetrics(
    ...
    const totalSize = Array.from(structure.filesByExtension.entries()).reduce(
      (sum, [ext, count]) => sum + count,
      0
    );
  ...
  private countOutdatedDependencies(dependencies: DependencyInfo[]): number {
    ...
    return dependencies.filter(
      dep => dep.version.includes('^') || dep.version.includes('~') || dep.version === 'latest'
    ).length;
  }

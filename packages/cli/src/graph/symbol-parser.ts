...
import { Project, SourceFile, Node, ImportDeclaration, CallExpression, ClassDeclaration, InterfaceDeclaration, FunctionDeclaration } from 'ts-morph';
...
export class SymbolParser {
   ...
   */
  async parseProject(projectPath: string, options: {
   ...
   */
  private async createExportSymbol(
    exportDecl: any, 
    filePath: string
  ): Promise<SymbolNode | null> {
    try {
    ...
    for (const [nodeId, node] of nodes) {
      if (node.type === 'class') {
        ...
        for (const parent of inheritance) {
          for (const [targetId, targetNode] of nodes) {
            if (targetNode.name === parent && (targetNode.type === 'class' || targetNode.type === 'interface')) {
              const edgeId = `${nodeId}->${targetId}`;
              edges.set(edgeId, {
                id: edgeId,
                source: nodeId,
  ...
  private async findSourceFiles(projectPath: string, options: any): Promise<string[]> {
    ...
    const scanDir = async (dir: string) => {
      try {
        ...
        this.logger.warn(`Could not access ${dir}: ${error instanceof Error ? error.message : String(error)}`);
      }
    };

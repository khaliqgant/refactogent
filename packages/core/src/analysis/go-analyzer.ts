...
export class GoAnalyzer extends ASTAnalyzer {
  ...
  async extractSymbols(filePath: string): Promise<CodeSymbol[]> {
    ...
    for (const match of structMatches) {
      ...
      const safeName = this.safeString(structName);
      symbols.push({
        name: safeName,
        fullyQualifiedName: `${filePath}:${safeName}`,
  ...
  private isGoExported(name?: string): boolean {
    ...
    return name ? name.length > 0 && name[0] >= 'A' && name[0] <= 'Z' : false;
  }

  private safeString(value?: string, fallback = 'unknown'): string {

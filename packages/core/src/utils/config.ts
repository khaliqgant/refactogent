...
import { ExtendedRefactogentConfig, ProjectType } from '../types/index.js';
import { Logger } from './logger.js';

const DEFAULT_CONFIG: ExtendedRefactogentConfig = {
  project: {
    type: 'unknown',
    rootPath: '.',
...
export class ConfigLoader {
   ...
   */
  async loadConfig(projectPath: string): Promise<ExtendedRefactogentConfig> {
    const configPaths = [
   ...
   */
  private parseConfigFile(configPath: string): ExtendedRefactogentConfig {
    try {
      const content = fs.readFileSync(configPath, 'utf8');
      const parsed = yaml.load(content) as Partial<ExtendedRefactogentConfig>;
   ...
   */
  private mergeWithDefaults(userConfig: Partial<ExtendedRefactogentConfig>): ExtendedRefactogentConfig {
    return {
      ...DEFAULT_CONFIG,
      ...userConfig,
      gates: {
   ...
   */
  private validateConfig(config: ExtendedRefactogentConfig): void {
    if (!config.version) {
   ...
   */
  getDefaultConfigForProject(projectType: ProjectType): ExtendedRefactogentConfig {

...
export interface RefactoringCandidate {
  filePath: string;
  safetyScore: number;
  refactoringType: 'extract_function' | 'rename_symbol' | 'move_file' | 'inline_function' | 'extract_class';
  description: string;
...
export class SafetyScorer {
  ...
  async generateRiskProfile(
    ...
    const files = projectAST.modules.map(module => this.assessFileRisk(module, apiEndpoints, coverageData, changeFrequencyData));
  ...
  private calculateTestCoverageMetric(projectAST: ProjectAST, coverageData?: CoverageData[]): SafetyMetric {
    if (!coverageData || coverageData.length === 0) {
      ...
      const estimatedCoverage = sourceFiles.length > 0 ? Math.min(100, (testFiles.length / sourceFiles.length) * 60) : 0;
   ...
   */
  private calculateApiExposureMetric(projectAST: ProjectAST, apiEndpoints: APIEndpoint[]): SafetyMetric {
   ...
   */
  private calculateChangeFrequencyMetric(projectAST: ProjectAST, changeFrequencyData?: ChangeFrequencyData[]): SafetyMetric {
    ...
    const avgChangesPerMonth = changeFrequencyData.reduce((sum, c) => sum + c.changesLastMonth, 0) / changeFrequencyData.length;
  ...
  private generateRecommendations(metrics: {
    ...
    if (metrics.complexity.score < 60) {
      recommendations.push({
        ...
        description: 'High code complexity increases the risk of introducing bugs during refactoring',
    ...
    if (metrics.apiExposure.score < 60) {
      recommendations.push({
        ...
        description: 'Large API surface area increases maintenance complexity and breaking change risk',
  ...
  private assessFileRisk(
    ...
    const riskScore = 100 - this.calculateFileScore({
      complexity,
      testCoverage,
      apiExposure,
      changeFrequency,
  ...
  private calculateGlobalMetrics(files: FileRiskAssessment[], projectAST: ProjectAST): GlobalRiskMetrics {
  ...
  private identifySafeRefactoringCandidates(files: FileRiskAssessment[], projectAST: ProjectAST): RefactoringCandidate[] {
    return files
  ...
  private getFileApiExposure(module: ModuleAST, apiEndpoints: APIEndpoint[]): number {
    const fileEndpoints = apiEndpoints.filter(endpoint => endpoint.location.file === module.filePath);
    return module.exports.length + fileEndpoints.length;
  }

  private getFileChangeFrequency(filePath: string, changeFrequencyData?: ChangeFrequencyData[]): number {

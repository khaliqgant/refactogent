...
export class APISurfaceDetector {
   ...
   */
  async detectAPISurface(projectPath: string, projectAST: Map<string, ProjectAST>): Promise<APISurface> {
    ...
    // Calculate summary
    const summary = this.calculateAPISummary(endpoints, httpRoutes, cliCommands, exportedFunctions, exportedClasses);
  ...
  private detectFlaskRoutes(content: string, filePath: string): HTTPRoute[] {
    const routes: HTTPRoute[] = [];
    const routePattern = /@app\.route\(['"`]([^'"`]+)['"`](?:,\s*methods\s*=\s*\[([^\]]+)\])?\)\s*\ndef\s+(\w+)/g;
  ...
  private detectCommanderCommands(content: string, filePath: string): CLICommand[] {
    const commands: CLICommand[] = [];
    const commandPattern = /\.command\(['"`]([^'"`]+)['"`]\)\s*\.description\(['"`]([^'"`]+)['"`]\)/g;
  ...
  private detectFastifyRoutes(content: string, filePath: string): HTTPRoute[] { return []; }
  private detectDjangoRoutes(content: string, filePath: string): HTTPRoute[] { return []; }
  private detectGorillaRoutes(content: string, filePath: string): HTTPRoute[] { return []; }
  private detectYargsCommands(content: string, filePath: string): CLICommand[] { return []; }
  private detectArgparseCommands(content: string, filePath: string): CLICommand[] { return []; }
  private detectCobraCommands(content: string, filePath: string): CLICommand[] { return []; }
  private detectFlagCommands(content: string, filePath: string): CLICommand[] { return []; }
  ...
  private convertCLICommandsToEndpoints(commands: CLICommand[]): APIEndpoint[] {
    return commands.map(cmd => ({
      ...
      parameters: cmd.options?.map(opt => ({
        name: opt.name,
        type: opt.type,

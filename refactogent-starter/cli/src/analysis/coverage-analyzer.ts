...
export class CoverageAnalyzer {
   ...
   */
  private async findExistingCoverage(projectPath: string, projectType: ProjectType): Promise<string | null> {
   ...
   */
  private async generateJavaScriptCoverage(projectPath: string, options: CoverageOptions): Promise<string> {
    try {
   ...
   */
  private async generatePythonCoverage(projectPath: string, options: CoverageOptions): Promise<string> {
    try {
   ...
   */
  private async parseCoverageData(coverageData: string, projectPath: string): Promise<FileCoverageInfo[]> {
    // Detect format
  ...
  private parseJsonCoverage(data: string, projectPath: string): FileCoverageInfo[] {
    try {
      ...
      for (const [filePath, fileInfo] of Object.entries(fileData)) {
        ...
        const linesCovered = Object.values(info.s || info.statementMap || {}).filter((hits: any) => hits > 0).length;
        const totalLines = Object.keys(info.s || info.statementMap || {}).length;
        const branchesCovered = Object.values(info.b || info.branchMap || {}).flat().filter((hits: any) => hits > 0).length;
        const totalBranches = Object.values(info.b || info.branchMap || {}).flat().length;
        const functionsCovered = Object.values(info.f || info.fnMap || {}).filter((hits: any) => hits > 0).length;
  ...
  private parseXmlCoverage(data: string, projectPath: string): FileCoverageInfo[] {
    ...
    const fileMatches = data.matchAll(/<class.*?name="([^"]+)".*?line-rate="([^"]+)".*?branch-rate="([^"]+)"/g);
  ...
  private calculateOverallMetrics(fileCoverage: FileCoverageInfo[]): CoverageMetrics {
    ...
    return {
      ...
      branchPercentage: totals.totalBranches > 0 ? (totals.branchesCovered / totals.totalBranches) * 100 : 0,
      functionPercentage: totals.totalFunctions > 0 ? (totals.functionsCovered / totals.totalFunctions) * 100 : 0,
      statementPercentage: totals.totalStatements > 0 ? (totals.statementsCovered / totals.totalStatements) * 100 : 0
    };
  }
   ...
   */
  private generateCoverageSummary(fileCoverage: FileCoverageInfo[], overallCoverage: CoverageMetrics): CoverageSummary {
    ...
    const poorlyCoveredFiles = fileCoverage.filter(f => f.metrics.linePercentage < 50 && f.metrics.linePercentage > 0).length;
    const uncoveredFiles = fileCoverage.filter(f => f.metrics.linePercentage === 0).length;
    
    const averageCoverage = fileCoverage.length > 0 
    ...
    const distribution: CoverageDistribution = {
      ...
      good: fileCoverage.filter(f => f.metrics.linePercentage >= 70 && f.metrics.linePercentage < 90).length,
      fair: fileCoverage.filter(f => f.metrics.linePercentage >= 50 && f.metrics.linePercentage < 70).length,
      poor: fileCoverage.filter(f => f.metrics.linePercentage >= 30 && f.metrics.linePercentage < 50).length,
   ...
   */
  private generateRiskAssessment(fileCoverage: FileCoverageInfo[], overallCoverage: CoverageMetrics): CoverageRiskAssessment {
    ...
    if (fileCoverage.filter(f => f.metrics.branchPercentage < 50).length > 0) {
      recommendations.push({
        ...
        files: fileCoverage.filter(f => f.metrics.branchPercentage < 50).map(f => f.filePath).slice(0, 5),
  ...
  private async performRegressionAnalysis(
    ...
    if (previousCoverage) {
      ...
      const functionChange = currentCoverage.functionPercentage - previousCoverage.functionPercentage;
  ...
  private generateFileRecommendations(metrics: CoverageMetrics, uncoveredLines: number[]): string[] {
    const recommendations: string[] = [];

...
export class TypeScriptAnalyzer extends ASTAnalyzer {
  ...
  private extractClassInfo(cls: any, sourceFile: SourceFile): ClassInfo {
    const methods: MethodInfo[] = cls.getMethods().map((method: any) => 
      this.extractMethodInfo(method, sourceFile)
    );

    const properties: PropertyInfo[] = cls.getProperties().map((prop: any) => 
      this.extractPropertyInfo(prop, sourceFile)
    );
  ...
  private extractInterfaceInfo(iface: any, sourceFile: SourceFile): InterfaceInfo {
    const methods: MethodInfo[] = iface.getMethods().map((method: any) => 
      this.extractMethodInfo(method, sourceFile)
    );

    const properties: PropertyInfo[] = iface.getProperties().map((prop: any) => 
      this.extractPropertyInfo(prop, sourceFile)
    );

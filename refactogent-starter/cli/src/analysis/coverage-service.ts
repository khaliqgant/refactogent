...
export class CoverageService {
  ...
  async monitorCoverageRegression(
    ...
    if (hasRegression && report.regressionAnalysis) {
      ...
      const significantRegressions = changes.filter(c => 
        c.type === 'regression' && Math.abs(c.impact) >= threshold
      );

      if (significantRegressions.length > 0) {
        recommendations.push(`${significantRegressions.length} files have significant coverage regression`);
  ...
  private generateJsonVisualization(report: CoverageReport): any {
    return {
      ...
      trends: report.regressionAnalysis ? {
        ...
        change: report.regressionAnalysis.changes.find(c => c.filePath === 'overall')?.impact || 0,
...
<body>
    ...
    <div class="recommendations">
        ...
        <ul>
            ${report.summary.riskAssessment.recommendations.map(rec => `
   ...
   */
  private generateActionItems(report: CoverageReport, safetyImpact: CoverageSafetyImpact): CoverageActionItem[] {
    ...
    if (report.regressionAnalysis?.regressionRisk !== 'none') {
      actionItems.push({
        ...
        files: report.regressionAnalysis?.changes

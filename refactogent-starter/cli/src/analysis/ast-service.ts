...
export class ASTService {
   ...
   */
  async analyzeProject(projectPath: string, projectType: ProjectType): Promise<UnifiedProjectAnalysis> {
  ...
  private generateRecommendations(
    ...
    if (metrics.architecturalScore < 70) {
      recommendations.push({
        ...
        suggestion: 'Review code organization, reduce complexity, and eliminate circular dependencies.'
      });
    }
    ...
    for (const [language, ast] of astByLanguage) {
      if (ast.metrics.circularDependencies.length > 0) {
        recommendations.push({
          ...
          suggestion: 'Refactor code to eliminate circular dependencies by introducing interfaces or reorganizing modules.'
        });
      }
    }
    ...
    if (apiSurface.summary.riskScore > 70) {
      recommendations.push({
        ...
        suggestion: 'Review API endpoints for over-exposure, consider consolidating similar endpoints, and implement proper access controls.'
      });
    }
    ...
    if (apiSurface.summary.httpEndpoints > 20) {
      recommendations.push({
        ...
        suggestion: 'Consider grouping related endpoints, implementing API versioning, and documenting all endpoints.'
      });
    }
    ...
    if (apiSurface.summary.frameworks.length > 2) {
      recommendations.push({
        ...
        suggestion: 'Consider standardizing on fewer frameworks to reduce complexity and maintenance overhead.'
      });
    }

...
`;
  ...
  it('should respect max-suggestions limit', () => {
    const result = execSync(`node ${cliPath} refactor-suggest ${testProjectDir} --max-suggestions 3 --format json`, { 
      encoding: 'utf8',
      cwd: path.dirname(cliPath),
      timeout: 30000
    });
  ...
  it('should handle different skill levels', () => {
    const beginnerResult = execSync(`node ${cliPath} refactor-suggest ${testProjectDir} --skill-level beginner --format json`, { 
      encoding: 'utf8',
      cwd: path.dirname(cliPath),
      timeout: 30000
    });

    const advancedResult = execSync(`node ${cliPath} refactor-suggest ${testProjectDir} --skill-level advanced --format json`, { 
      encoding: 'utf8',
      cwd: path.dirname(cliPath),
      timeout: 30000
    });
  ...
  it('should handle different prioritization options', () => {
    ...
    for (const priority of priorities) {
      const result = execSync(`node ${cliPath} refactor-suggest ${testProjectDir} --prioritize ${priority} --format json`, { 
        encoding: 'utf8',
        cwd: path.dirname(cliPath),
        timeout: 30000
      });
  ...
  it('should show quick wins when requested', () => {
    const result = execSync(`node ${cliPath} refactor-suggest ${testProjectDir} --quick-wins-only`, { 
      encoding: 'utf8',
      cwd: path.dirname(cliPath),
      timeout: 30000
    });
  ...
  it('should save output to file when specified', async () => {
    ...
    execSync(`node ${cliPath} refactor-suggest ${testProjectDir} --format json --output ${outputFile}`, { 
      encoding: 'utf8',
      cwd: path.dirname(cliPath),
      timeout: 30000
    });

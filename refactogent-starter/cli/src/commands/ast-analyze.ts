...
export class ASTAnalyzeCommand extends BaseCommand {
  async execute(options: ASTAnalyzeOptions): Promise<any> {
    try {
      ...
      return this.failure(`AST analysis failed: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  private displaySummary(analysis: any, verbose: boolean): void {
    ...
    console.log(`  Frameworks: ${analysis.apiSurface.summary.frameworks.join(', ') || 'None detected'}`);
    ...
    if (analysis.recommendations.length > 0) {
      ...
      if (!verbose && analysis.recommendations.length > 3) {
        console.log(`  ... and ${analysis.recommendations.length - 3} more (use --verbose for full list)`);
      }
    }
    ...
    if (verbose) {
      ...
      if (hotspots.length > 0) {
        ...
        hotspots.slice(0, 5).forEach(symbol => {
          console.log(`  ${symbol.name} (${symbol.type}) - ${symbol.location.file}:${symbol.location.startLine}`);
        });
      }
...
<body>
    ...
    <div class="section">
        ...
        ${Array.from(analysis.astByLanguage.entries()).map((entry: any) => {
    ...
    <div class="section">
        ...
        <table>
            ...
            ${this.getSymbolSummary(analysis.unifiedSymbols).map(([type, count, publicCount]: [string, number, number]) => `
                <tr>
                    <td>${type}</td>
                    <td>${count}</td>
                    <td>${publicCount}</td>

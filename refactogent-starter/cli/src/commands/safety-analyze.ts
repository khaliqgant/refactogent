  ...
  command
    ...
    .action(async (projectPath: string, options) => {
      ...
      try {
        ...
        const safetyScore = await safetyScorer.calculateProjectSafety(primaryAST, apiSurface.endpoints);
        const riskProfile = await safetyScorer.generateRiskProfile(primaryAST, apiSurface.endpoints);
        ...
        const result = {
          ...
          riskProfile,
          summary: {
            overallRating: safetyScore.overall >= 90 ? 'excellent' : 
                          safetyScore.overall >= 75 ? 'good' : 
                          safetyScore.overall >= 60 ? 'fair' : 
                          safetyScore.overall >= 40 ? 'poor' : 'critical',
            ...
            quickWins: riskProfile.safeRefactoringCandidates.length > 0 ? ['Safe refactoring opportunities'] : [],
            riskLevel: safetyScore.overall >= 80 ? 'low' : 
                      safetyScore.overall >= 60 ? 'medium' : 
                      safetyScore.overall >= 40 ? 'high' : 'critical',
            readinessForRefactoring: safetyScore.overall > 60 && safetyScore.testCoverage.score > 50
          }
        };
...
async function outputJson(result: any, outputFile?: string, logger?: Logger): Promise<void> {
  ...
  if (outputFile) {
    try {
      ...
      logger?.error('Failed to write file', { outputFile, error: error instanceof Error ? error.message : String(error) });
...
function outputConsole(result: any, logger: Logger): void {
  ...
  console.log(`✅ Ready for Refactoring: ${result.summary.readinessForRefactoring ? 'YES' : 'NO'}\n`);

  // Detailed Scores
  console.log('📈 Detailed Scores:');
  console.log(`   Complexity:        ${result.safetyScore.complexity.score.toFixed(1)}/100 (${result.safetyScore.complexity.riskLevel})`);
  console.log(`   Test Coverage:     ${result.safetyScore.testCoverage.score.toFixed(1)}/100 (${result.safetyScore.testCoverage.riskLevel})`);
  console.log(`   API Exposure:      ${result.safetyScore.apiExposure.score.toFixed(1)}/100 (${result.safetyScore.apiExposure.riskLevel})`);
  console.log(`   Dependency Risk:   ${result.safetyScore.dependencyRisk.score.toFixed(1)}/100 (${result.safetyScore.dependencyRisk.riskLevel})`);
  console.log(`   Change Frequency:  ${result.safetyScore.changeFrequency.score.toFixed(1)}/100 (${result.safetyScore.changeFrequency.riskLevel})\n`);
  ...
  if (result.riskProfile.riskHotspots && result.riskProfile.riskHotspots.length > 0) {
    ...
    result.riskProfile.riskHotspots.slice(0, 5).forEach((hotspot: any, index: number) => {
      console.log(`   ${index + 1}. ${path.basename(hotspot.filePath)} (Risk: ${hotspot.riskScore.toFixed(1)})`);
  ...
  if (result.riskProfile.safeRefactoringCandidates && result.riskProfile.safeRefactoringCandidates.length > 0) {
    console.log('✨ Safe Refactoring Opportunities (Top 3):');
    result.riskProfile.safeRefactoringCandidates.slice(0, 3).forEach((candidate: any, index: number) => {
      console.log(`   ${index + 1}. ${candidate.refactoringType.replace('_', ' ')} in ${path.basename(candidate.filePath)}`);
      console.log(`      Safety: ${candidate.safetyScore.toFixed(1)}/100, Effort: ${candidate.estimatedEffort}`);
  ...
  topRecommendations.forEach((rec: any, index: number) => {
    ...
    if (rec.suggestedActions && rec.suggestedActions.length > 0) {
      console.log(`      Actions: ${rec.suggestedActions.slice(0, 2).join(', ')}${rec.suggestedActions.length > 2 ? '...' : ''}`);
    }
    console.log();
  });
  ...
  console.log(`   Average Complexity:     ${result.riskProfile.globalMetrics.averageComplexity.toFixed(1)}`);
  console.log(`   Total Test Coverage:    ${result.riskProfile.globalMetrics.totalTestCoverage.toFixed(1)}%`);
  console.log(`   Public API Surface:     ${result.riskProfile.globalMetrics.publicApiSurface} items`);
  console.log(`   Dependency Health:      ${result.riskProfile.globalMetrics.dependencyHealth.toFixed(1)}/100`);
  console.log(`   Architectural Debt:     ${result.riskProfile.globalMetrics.architecturalDebt.toFixed(1)}%`);
  console.log();
...
${result.summary.keyStrengths.length > 0 ? `**Strengths:**
${result.summary.keyStrengths.map((s: string) => `- ✅ ${s}`).join('\n')}

` : ''}${result.summary.majorConcerns.length > 0 ? `**Major Concerns:**
${result.summary.majorConcerns.map((c: string) => `- ⚠️ ${c}`).join('\n')}

` : ''}${result.summary.quickWins.length > 0 ? `**Quick Wins:**
...
${result.riskProfile.riskHotspots.length > 0 ? 
  result.riskProfile.riskHotspots.slice(0, 10).map((hotspot: any, index: number) => 
...
${result.riskProfile.safeRefactoringCandidates.length > 0 ?
  result.riskProfile.safeRefactoringCandidates.slice(0, 5).map((candidate: any, index: number) =>

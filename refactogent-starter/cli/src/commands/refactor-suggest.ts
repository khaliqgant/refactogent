...
// import { SafetyService } from '../analysis/safety-service.js';
...
const MOCK_SAFETY_SCORES = {
  OVERALL: 75,
  COMPLEXITY: { score: 70, weight: 0.25, details: 'Moderate complexity', riskLevel: 'medium' as const },
  TEST_COVERAGE: { score: 80, weight: 0.30, details: 'Good coverage', riskLevel: 'low' as const },
  API_EXPOSURE: { score: 85, weight: 0.20, details: 'Reasonable API surface', riskLevel: 'low' as const },
  DEPENDENCY_RISK: { score: 90, weight: 0.15, details: 'Low dependency risk', riskLevel: 'low' as const },
  CHANGE_FREQUENCY: { score: 95, weight: 0.10, details: 'Stable changes', riskLevel: 'low' as const }
} as const;
...
export function createRefactorSuggestCommand(): Command {
  const command = new Command('refactor-suggest')
    ...
    .option('-p, --prioritize <criteria>', 'Prioritization criteria (safety|impact|effort|readiness)', DEFAULT_PRIORITIZE)
    .option('-m, --max-suggestions <number>', 'Maximum number of suggestions', DEFAULT_MAX_SUGGESTIONS.toString())
    .option('-s, --skill-level <level>', 'Skill level (beginner|intermediate|advanced)', DEFAULT_SKILL_LEVEL)
    ...
    .option('--focus-areas <areas>', 'Comma-separated list of file patterns or directories to focus on')
...
async function executeRefactorSuggest(
  ...
  try {
    ...
    const coverageResult = await coverageService.analyzeCoverageWithIntegration(resolvedPath, projectType);
  ...
  const maxSuggestions = options.maxSuggestions ? parseInt(options.maxSuggestions, 10) : DEFAULT_MAX_SUGGESTIONS;
...
async function outputResults(
  ...
  if (options.format === 'detailed' && suggestions.length > 0) {
    ...
    topSuggestions.forEach((suggestion: any, index: number) => {
      ...
      console.log(`   ${opportunity.description}`);
      console.log(`   📍 Location: ${opportunity.location.filePath}:${opportunity.location.startLine}-${opportunity.location.endLine}`);
      ...
      if (opportunity.risks.length > 0) {
        const highRisks = opportunity.risks.filter((r: any) => r.severity === 'high' || r.severity === 'critical');

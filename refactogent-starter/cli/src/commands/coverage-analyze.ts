  ...
  command
    ...
    .action(async (projectPath: string, options) => {
      ...
      try {
        ...
        const exitCode = result.report.overallCoverage.linePercentage < parseInt(options.threshold) ? 1 : 0;
...
async function outputHtml(result: any, outputFile?: string, logger?: Logger): Promise<void> {
  ...
  try {
    ...
    logger?.error('Failed to write HTML file', { outputFile: fileName, error: error instanceof Error ? error.message : String(error) });
...
function outputConsole(result: any, logger: Logger): void {
  ...
  console.log(`   Lines:      ${report.overallCoverage.linePercentage.toFixed(1)}% (${report.overallCoverage.linesCovered}/${report.overallCoverage.totalLines})`);
  console.log(`   Branches:   ${report.overallCoverage.branchPercentage.toFixed(1)}% (${report.overallCoverage.branchesCovered}/${report.overallCoverage.totalBranches})`);
  console.log(`   Functions:  ${report.overallCoverage.functionPercentage.toFixed(1)}% (${report.overallCoverage.functionsCovered}/${report.overallCoverage.totalFunctions})`);
  console.log(`   Statements: ${report.overallCoverage.statementPercentage.toFixed(1)}% (${report.overallCoverage.statementsCovered}/${report.overallCoverage.totalStatements})\n`);
  ...
  if (safetyImpact.criticalFiles.length > 0) {
    console.log(`   Critical Files:      ${safetyImpact.criticalFiles.length} files need attention`);
  }
  console.log();
  ...
  if (worstFiles.length > 0) {
    ...
    worstFiles.forEach((file: any, index: number) => {
      const riskIcon = file.riskLevel === 'critical' ? '🔴' : 
                      file.riskLevel === 'high' ? '🟠' : 
                      file.riskLevel === 'medium' ? '🟡' : '🟢';
      console.log(`   ${index + 1}. ${riskIcon} ${path.basename(file.filePath)} - ${file.metrics.linePercentage.toFixed(1)}%`);
  ...
  if (report.regressionAnalysis) {
    ...
    if (regression.previousCoverage) {
      const change = regression.currentCoverage.linePercentage - regression.previousCoverage.linePercentage;
      const changeIcon = change >= 0 ? '📈' : '📉';
      console.log(`   Previous Coverage: ${regression.previousCoverage.linePercentage.toFixed(1)}%`);
      console.log(`   Current Coverage:  ${regression.currentCoverage.linePercentage.toFixed(1)}%`);
      console.log(`   Change: ${changeIcon} ${change >= 0 ? '+' : ''}${change.toFixed(1)} percentage points`);
  ...
  if (actionItems.length > 0) {
    ...
    actionItems.slice(0, 5).forEach((item: any, index: number) => {
      ...
      console.log(`   ${index + 1}. ${priorityIcon} ${item.title} (${item.priority.toUpperCase()})`);
      console.log(`      ${item.description}`);
      console.log(`      Estimated effort: ${item.estimatedHours} hours`);
      console.log(`      Impact: ${item.impact}`);
      if (item.files.length > 0) {
        console.log(`      Files: ${item.files.slice(0, 3).map((f: string) => path.basename(f)).join(', ')}${item.files.length > 3 ? '...' : ''}`);
      }
      console.log();
    });
  }
  ...
  // Summary Assessment
  const overallRisk = report.overallCoverage.linePercentage >= 80 ? 'LOW' :
                     report.overallCoverage.linePercentage >= 60 ? 'MEDIUM' :
                     report.overallCoverage.linePercentage >= 30 ? 'HIGH' : 'CRITICAL';

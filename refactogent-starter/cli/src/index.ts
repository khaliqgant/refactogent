...
program.command('plan')
  ...
  .option('--mode <mode>', 'Refactoring mode: organize-only | name-hygiene | tests-first | micro-simplify', 'organize-only')
  .action(async (opts, command) => {
    const globalOpts = command.parent.opts();
    const context = await initializeCLI(globalOpts);
  ...
  .action(async (opts, command) => {
    ...
    if (result.success) {
      ...
      if (result.data?.projectCount) {
        console.log(`📁 Generated ${result.data.projectCount} projects in ${result.data.outputDir}`);

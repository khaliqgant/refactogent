...
export class TestEnvironment {
  ...
  constructor(config: TestEnvironmentConfig, logger: Logger) {
    this.config = config;
    this.logger = logger;
    this.workingDir = path.join(process.cwd(), '.refactogent', 'test-environments', this.generateId());
  }
  ...
  async captureState(): Promise<TestEnvironmentState> {
    ...
    fs.writeFileSync(state.snapshotPath, JSON.stringify({
      ...state,
   ...
   */
  compareStates(before: TestEnvironmentState, after: TestEnvironmentState): {
    filesChanged: string[];
    filesAdded: string[];
    filesRemoved: string[];
  ...
  private async setupDockerEnvironment(): Promise<void> {
    ...
    const buildResult = await this.executeCommand('docker', [
      'build', '-t', `refactogent-test-${this.generateId()}`, '.'
    ]);
    ...
    const runResult = await this.executeCommand('docker', [
      'run', '-d', '--name', `refactogent-test-${this.generateId()}`,
      '--memory', this.config.resources.memory,
      '--cpus', this.config.resources.cpu,

...
import { TestEnvironment, TestEnvironmentConfig, TestEnvironmentState, TestResult } from './test-environment.js';
...
export class TestHarness {
  ...
  async executeSuite(suite: TestSuite): Promise<TestHarnessResult> {
    ...
    try {
      ...
      result.summary.coverage = result.totalTests > 0 ? (result.passedTests / result.totalTests) * 100 : 0;
   ...
   */
  private async executeEnvironment(config: TestEnvironmentConfig, tests: TestCase[]): Promise<TestEnvironmentResult> {
   ...
   */
  private async executeTestCase(environment: TestEnvironment, testCase: TestCase): Promise<TestCaseResult> {
   ...
   */
  private async runAssertion(environment: TestEnvironment, assertion: TestAssertion, output: string): Promise<TestAssertion> {
    ...
    try {
      switch (assertion.type) {
        ...
        case 'fileContent':
          const { file, content } = assertion.expected as { file: string; content: string | RegExp };
    ...
    if (fs.existsSync(previousResultsPath)) {
      try {
        ...
        if (previousResults.passedTests > result.passedTests) {
          result.summary.regressions.push(`Test count decreased from ${previousResults.passedTests} to ${result.passedTests}`);
        } else if (previousResults.passedTests < result.passedTests) {
          result.summary.improvements.push(`Test count increased from ${previousResults.passedTests} to ${result.passedTests}`);
        }

        if (previousResults.summary.coverage > result.summary.coverage) {
          result.summary.regressions.push(`Coverage decreased from ${previousResults.summary.coverage}% to ${result.summary.coverage}%`);
        } else if (previousResults.summary.coverage < result.summary.coverage) {
          result.summary.improvements.push(`Coverage increased from ${previousResults.summary.coverage}% to ${result.summary.coverage}%`);
        }
  ...
  private async cleanupAllEnvironments(): Promise<void> {
    ...
    const cleanupPromises = Array.from(this.activeEnvironments.values()).map(env => 
      env.cleanup().catch(error => 
        this.logger.error('Failed to cleanup environment', { error })
      )
    );
      ...
      tests: [
        {
          ...
          commands: projectType === 'typescript' ? ['npm run build'] : 
                   projectType === 'python' ? ['python -m py_compile *.py'] :
                   projectType === 'go' ? ['go build'] : ['echo "No build command"'],
          assertions: [
        ...
        },
        {
          ...
          commands: projectType === 'typescript' ? ['npm test'] :
                   projectType === 'python' ? ['python -m pytest'] :
                   projectType === 'go' ? ['go test ./...'] : ['echo "No test command"'],
          assertions: [

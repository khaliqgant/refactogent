...
import { PatternDetector, RefactoringOpportunity, DetectionResult, PatternDetectionOptions } from './pattern-detector.js';
...
export class SuggestionEngine {
  ...
  private calculatePriority(
    ...
    if (safetyScore.overall < 60 && opportunity.pattern.category === 'simplify') {
      return 'critical';
    }
  ...
  private async assessReadiness(
    ...
    if (coveragePrereq) {
      ...
      prerequisites.push({
        ...
        howToResolve: hasCoverage ? undefined : 'Add unit tests for the code being refactored'
      });
  ...
  private estimateTimeline(
    ...
    const estimatedDays = effortDays[opportunity.estimatedEffort] * 
    ...
    return suggestions.sort((a, b) => {
      switch (priorityBy) {
        ...
        case 'effort':
          ...
          return effortOrder[b.opportunity.estimatedEffort] - effortOrder[a.opportunity.estimatedEffort];
        case 'readiness':
  ...
  private generateSummary(suggestions: RefactoringSuggestion[]): SuggestionSummary {
    ...
    const averageImpact = suggestions.length > 0 
      ? suggestions.reduce((sum, s) => sum + s.impact.overallBenefit, 0) / suggestions.length 
      : 0;
    
    const averageRisk = suggestions.length > 0
      ? suggestions.reduce((sum, s) => sum + s.impact.riskScore, 0) / suggestions.length
      : 0;
    
    const estimatedTotalHours = suggestions.reduce((sum, s) => sum + s.implementation.totalEstimatedHours, 0);
  ...
  private generateRecommendations(
    ...
    if (suggestions.length === 0) {
      recommendations.push('No refactoring suggestions available. Code appears to be well-structured.');
      return recommendations;
    }

    const quickWins = suggestions.filter(s => s.timeline.quickWin);
    if (quickWins.length > 0) {
      recommendations.push(`Start with ${quickWins.length} quick win opportunities for immediate impact`);
    }
  ...
  private getFileCoverage(filePath: string, coverageReport?: CoverageReport): number {
    ...
    const fileCoverage = coverageReport.fileCoverage.find((f: FileCoverageInfo) => f.filePath === filePath);
    return fileCoverage ? fileCoverage.metrics.linePercentage : 0;
  }

  private calculateRiskLevel(opportunity: RefactoringOpportunity, blockerCount: number): 'low' | 'medium' | 'high' | 'critical' {
